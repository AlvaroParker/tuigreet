...
impl Greeter {
    pub fn parse_debug(&mut self) {
        if self.config().opt_present("debug") || self.config.defaults.debug {
            ...
            self.logfile = match self
                .config()
                .opt_str("debug")
                .or_else(|| self.config.defaults.log_file.clone())
            {
    ...
    pub fn parse_theme(&mut self) -> Result<(), Box<dyn Error>> {
        if let Some(spec) = self
            .config()
            .opt_str("theme")
            .or_else(|| self.config.ui.theme.clone())
        {
    ...
    pub fn parse_greeting(&mut self) -> Result<(), Box<dyn Error>> {
        let has_greeting =
            self.config().opt_present("greeting") || self.config.ui.greeting.is_some();
        ...
        self.greeting = self
            .option("greeting")
            .or_else(|| self.config.ui.greeting.clone());
    ...
    pub fn parse_asterisks(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        if has_asterisks {
            let asterisk = if let Some(value) = self
                .config()
                .opt_str("asterisks-char")
                .or_else(|| self.config.ui.asterisks_char.map(|c| c.to_string()))
            {
                if value.chars().count() < 1 {
                    return Err(
                        "--asterisks-char must have at least one character as its value".into(),
                    );
                }
    ...
    pub fn parse_default_command(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        if let Some(command) = self
            .option("cmd")
            .or_else(|| self.config.defaults.command.clone())
        {
            let envs = self
                .options_multi("env")
                .or_else(|| self.config.defaults.env.clone());
    ...
    pub fn parse_sessions(&mut self) {
        if let Some(dirs) = self.option("sessions") {
            ...
            self.session_paths
                .extend(dirs.into_iter().map(|dir| (dir, SessionType::Wayland)));
        }
        ...
        if let Some(dirs) = self.option("xsessions") {
            ...
            self.session_paths
                .extend(dirs.into_iter().map(|dir| (dir, SessionType::X11)));
        }

        if self.option("session-wrapper").is_some()
            || self.config.sessions.wayland_wrapper.is_some()
        {
            self.session_wrapper = self
                .option("session-wrapper")
                .or_else(|| self.config.sessions.wayland_wrapper.clone());
        }

        if !self.config().opt_present("no-xsession-wrapper")
            && !self.config.sessions.x11_wrapper_disabled
        {
    ...
    pub fn parse_time(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        if let Some(format) = self
            .config()
            .opt_str("time-format")
            .or_else(|| self.config.ui.time_format.clone())
        {
    ...
    pub fn parse_menus(&mut self) -> Result<(), Box<dyn Error>> {
        if self.config().opt_present("user-menu") || self.config.ui.show_user_menu {
            ...
            let min_uid = self
                ...
                .or(self.config.defaults.user_min_uid);
            let max_uid = self
                .config()
                .opt_str("user-menu-max-uid")
                .and_then(|uid| uid.parse::<u16>().ok())
                .or(self.config.defaults.user_max_uid);
            ...
            if min_uid >= max_uid {
                return Err(
                    "Minimum UID ({min_uid}) must be less than maximum UID ({max_uid})".into(),
                );
            }
    ...
    pub fn parse_remembers(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        let has_remember_session =
            self.config().opt_present("remember-session") || self.config.remember.last_session;
        let has_remember_user_session = self.config().opt_present("remember-user-session")
            || self.config.remember.last_user_session;
    ...
    pub fn parse_power(&mut self) {
        self.powers.options.push(Power {
            ...
            command: self
                ...
                .or_else(|| self.config.defaults.shutdown_command.clone()),
        });
        ...
        self.powers.options.push(Power {
            ...
            command: self
                ...
                .or_else(|| self.config.defaults.reboot_command.clone()),
        });

        self.power_setsid =
            !(self.config().opt_present("power-no-setsid") || self.config.defaults.power_no_setsid);
    }
    ...
    pub fn parse_keybinds(&mut self) -> Result<(), Box<dyn Error>> {
        self.kb_command = self
            ...
            .or(self.config.ui.command_f_key)
            .unwrap_or(2);
        self.kb_sessions = self
            .config()
            .opt_str("kb-sessions")
            .and_then(|i| i.parse::<u8>().ok())
            .or(self.config.ui.sessions_f_key)
            .unwrap_or(3);
        self.kb_power = self
            .config()
            .opt_str("kb-power")
            .and_then(|i| i.parse::<u8>().ok())
            .or(self.config.ui.power_f_key)
            .unwrap_or(12);
...
mod tests {
    ...
    async fn test_command_line_arguments() {
        type Table<'a> = [(&'a [&'a str], bool, Option<fn(&Greeter)>)];
        let table: &Table = &[
            // No arguments
            (&[], true, None),
            // Valid combinations
        ...
        for (args, valid, check) in table {
            ...
            match valid {
                true => {
                    ...
                    assert!(
                        matches!(greeter.parse_config().await, Ok(())),
                        "{:?} cannot be parsed",
                        greeter.opts
                    );
    ...
    async fn command_and_env() {
        ...
        type Table<'a> = [(&'a [&'a str], fn(&Greeter), fn(&Greeter))];
        let table: &Table = &[
            (
            ...
            (
                ...
                |greeter| {
                    assert!(
                        matches!(&greeter.session_source, SessionSource::DefaultCommand(cmd, Some(env)) if cmd == "filecmd" && env.len() == 1 && env.first().unwrap() == "FILEENV=value")
                    );
                },
                |greeter| {
                    assert!(
                        matches!(&greeter.session_source, SessionSource::DefaultCommand(cmd, Some(env)) if cmd == "mycommand" && env.len() == 2)
                    );
                },
            ),
            (
