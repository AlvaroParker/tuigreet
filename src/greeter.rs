...
use crate::{
    ...
    event::Event,
    info::{
        get_last_command, get_last_session_path, get_last_user_command, get_last_user_name,
        get_last_user_session, get_last_user_username, get_sessions,
    },
    ui::{
...
impl Greeter {
    ...
    pub fn window_padding(&self) -> u16 {
        if let Some(padding) = self
            .option("window-padding")
            .and_then(|value| value.parse::<u16>().ok())
            .or(self.config.ui.window_padding)
        {
            return padding;
        }
    ...
    pub fn container_padding(&self) -> u16 {
        if let Some(padding) = self
            ...
            .and_then(|value| value.parse::<u16>().ok())
            .or(self.config.ui.container_padding)
        {
            return padding + 1;
        }
    ...
    pub fn prompt_padding(&self) -> u16 {
        if let Some(padding) = self
            .option("prompt-padding")
            .and_then(|value| value.parse::<u16>().ok())
            .or(self.config.ui.prompt_padding)
        {
            return padding;
        }
    ...
    pub fn greet_align(&self) -> GreetAlign {
        if let Some(value) = self
            .option("greet-align")
            .or_else(|| self.config.ui.greet_align.clone())
        {
            match value.as_str() {
    ...
    pub fn options() -> Options {
        ...
        opts.optopt(
            "",
            "config",
            "Path to tuigreet's configuration file",
            "FILE",
        );
        opts.optflagopt(
            "d",
